...
record Cxt : Set₁ where
  constructor _▹_st_
  field
    pos : Set                       -- context: Ø ⊢ pos
    dir : pos -> Set                -- context: (p : pos) ⊢ dir 
    rel : (p : pos) → dir p → Set   -- context: (p : pos)(d : dir p) |- rel
    
    -- is this type just Contexts of length `n`?
    -- then these operations are similar to MLTT substitution calculus?
    

_⊎ₚ_ : Cxt → Cxt → Cxt
p ⊎ₚ q = (pos ⊎ pos') ▹ (λ {(inj₁ x) → dir x
                          ; (inj₂ y) → dir' y}) st λ{(inj₁ x) d → rel x d
                                                   ; (inj₂ y) d → rel' y d}
    where 
        open Cxt p 
        open Cxt q renaming (pos to pos'; dir to dir'; rel to rel')


_×ₚ_ : Cxt → Cxt → Cxt
p ×ₚ q = (pos × pos') ▹ (λ {(p , p') → dir p ⊎ dir' p'}) st λ{(p , p') (inj₁ d) → rel p d
                                                            ; (p , p') (inj₂ d') → rel' p' d'} 
    where 
        open Cxt p 
        open Cxt q renaming (pos to pos'; dir to dir'; rel to rel')
...
_⊗ₚ_ : Cxt → Cxt → Cxt
p ⊗ₚ q = (pos × pos') ▹ (λ{(x , y) → dir x × dir' y}) st λ{(p , p') (d , d') → rel p d × rel' p' d'}
    where 
        open Cxt p 
        open Cxt q renaming (pos to pos'; dir to dir'; rel to rel')


-- this instead becomes term of a context?
⦅_⦆ : Cxt → Set → Set 
⦅ P ▹ D st C ⦆ X = Σ[ p ∈ P ] (∀(d : D p) → C p d → X)
--Σ[ p ∈ P ] (D p → X)

record Cxt[_,_](p q : Cxt) : Set where
    constructor _⇒ₚ_st_
    open Cxt p 
    open Cxt q renaming (pos to pos'; dir to dir'; rel to rel')
    field
        onPos : pos → pos'                           -- forward
        onDir : (p : pos) → dir' (onPos p) → dir p   -- backward
        onRel : (p : pos) → (d : dir' (onPos p)) →   -- forward 
            (rel p (onDir p d) → rel' (onPos p) d)
        -- Dial, mapping of relations

_⇒∘ₚ_ : {p q r : Cxt} → Cxt[ p , q ] → Cxt[ q , r ] → Cxt[ p , r ]
_⇒∘ₚ_ {p} {q} {r} pq qr = (λ x → onPos' (onPos x)) 
                            ⇒ₚ 
                          (λ ppos → let 
                                        qpos = onPos ppos
                                        in onDir ppos ∘ onDir' qpos) 
                            st 
                          λ ppos rdir → let 
                                            qpos = onPos ppos
                                            qdir = onDir' qpos rdir
                                            prel⇒qrel = onRel ppos qdir
                                            qrel⇒rrel = onRel' qpos rdir
                                            in qrel⇒rrel ∘ prel⇒qrel
    where 
        open Cxt[_,_] pq 
        open Cxt[_,_] qr renaming(onPos to onPos'; onDir to onDir'; onRel to onRel')
